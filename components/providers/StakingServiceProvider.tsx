...
export function StakingServiceProvider({ token, children }: StakingServiceProviderProps) {
  // Render the appropriate provider based on token
  if (token === exoETH || token === wstETH) {
    return <EVMLSTStakingProvider token={token}>{children}</EVMLSTStakingProvider>;
  }
