...
export function WalletConnectionModal({
  ...
  // Extract connection status from staking service
  const { connectionStatus, token } = stakingService;
  const { isReady, issues, boundAddress } = connectionStatus;
  ...
  const handleNativeDisconnect = async () => {
    try {
      if (!evmCompatible && customConnector) {
  ...
  useEffect(() => {
    const checkCustomConnectorInstallation = async () => {
      if (customConnector?.checkInstallation) {
        const installed = await customConnector.checkInstallation();
