...
import {
  isValidPublicKey,
  isValidSignature,
  isValidDepositDataRoot,
} from "@/utils/nstEthValidation";
import {
  OperationProgress,
  OperationStep,
...
export function StakeNSTTab({
  ...
  if (sourceChain !== destinationChain) {
    throw new Error(
      "Source and destination chains must be the same for NST staking",
    );
  }
  ...
  const [currentStep, setCurrentStep] = useState<"capsule" | "stake">(
    "capsule",
  );
  ...
  const [currentOperationStep, setCurrentOperationStep] =
    useState<OperationStep | null>(null);
  ...
  const walletBalance = stakingService.tokenBalance;
  ...
  const {
    ...
    decimals: walletBalance?.balance.decimals || 0,
    maxAmount: walletBalance?.balance.value || BigInt(0),
  });

  // Add validation logic - match Bootstrap contract exactly
  const AFTER_PECTRA_MIN_ACTIVATION_BALANCE_ETH_PER_VALIDATOR = BigInt(
    "32000000000000000000",
  ); // 32 ETH
  const AFTER_PECTRA_MAX_EFFECTIVE_BALANCE_ETH_PER_VALIDATOR = BigInt(
  ...
  const isMultipleOfGwei =
    parsedAmount === BigInt(0) || parsedAmount % GWEI_UNIT === BigInt(0);
  ...
  if (isMultipleOfGwei && parsedAmount > BigInt(0)) {
    if (isPectraMode) {
      isValidAmount =
        parsedAmount >= AFTER_PECTRA_MIN_ACTIVATION_BALANCE_ETH_PER_VALIDATOR &&
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">
          First, create a capsule contract that represents the validator&apos;s
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="text-blue-300 text-sm">
            ...
            <ul className="space-y-1 text-blue-200">
              <li>
                • A smart contract that represents the validator&apos;s
                withdrawal credentials
              </li>
              <li>• Required for native staking to the beacon chain</li>
              <li>
                • Each ETH restaker gets a unique capsule address, which
                supports multiple validators
              </li>
              <li>
                • The capsule receives rewards and withdrawals from the beacon
                chain
              </li>
            </ul>
          </div>
      ...
      {currentOperationStep && (
        <OperationProgress
          ...
          onViewDetails={
            currentOperationStep?.txHash
              ? () => {
                  const currentTxHash = currentOperationStep?.txHash;
                  if (
                    stakingService.token.network.txExplorerUrl &&
                    currentTxHash
                  ) {
                    window.open(
                      `${stakingService.token.network.txExplorerUrl}${currentTxHash}`,
                      "_blank",
                    );
                  }
                }
  ...
  const renderStakeStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">
          Stake ETH to the beacon chain with your capsule as withdrawal
          credentials
          {isPectraMode ? " (32-2048 ETH range)" : " (exactly 32 ETH)"}
        </p>
      </div>
      ...
      {capsuleAddress && (
        <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <a
              href={`${stakingService.token.network.accountExplorerUrl}${capsuleAddress}`}
              target="_blank"
      ...
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-gray-400">
            Balance:{" "}
            {walletBalance ? formatUnits(walletBalance.balance.value, 18) : "0"}{" "}
            ETH
          </span>
          <button
            onClick={() => {
              const walletBalanceEth = Number(
                formatUnits(walletBalance?.balance.value || BigInt(0), 18),
              );
              let maxAmount;
              if (isPectraMode) {
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) =>
              setStakeData((prev) => ({
                ...prev,
                depositDataRoot: e.target.value,
              }))
            }
            className={
              stakeData.depositDataRoot &&
              !isValidDepositDataRoot(stakeData.depositDataRoot)
                ? "border-red-500"
                : ""
            }
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
        ...
        <ul className="text-yellow-300 text-sm space-y-1">
          <li>
            • Use the guide at{" "}
            <a
              href="https://deposit-cli.ethstaker.cc/landing.html"
              target="_blank"
              rel="noopener noreferrer"
            >
              https://deposit-cli.ethstaker.cc/
            </a>{" "}
            to generate the validator public key, signature, and deposit data
            root
          </li>
          <li>
          ...
          <li>
            • After staking, wait for the deposit to be activated on the beacon
            chain and verify it in the next tab
          </li>
        </ul>
      </div>
          ...
          onViewDetails={
            stakeData.pubkey
              ? () => {
                  ...
                  const validatorExplorerUrl = (
                    stakingService.token.network as {
                      validatorExplorerUrl?: string;
                    }
                  ).validatorExplorerUrl;
                  if (validatorExplorerUrl && stakeData.pubkey) {
                    window.open(
                      `${validatorExplorerUrl}${stakeData.pubkey}`,
                      "_blank",
                    );
                  }
                }
