...
export function VerifyTab({
  ...
  if (!bootstrapStatus?.isBootstrapped) {
    if (sourceChain !== destinationChain) {
      throw new Error(
        "Source and destination chains must be the same during bootstrap phase",
      );
    }
  } else {
  ...
  if (
    !(
      "validatorExplorerUrl" in stakingService.token.network &&
      stakingService.token.network.validatorExplorerUrl
    )
  ) {
  ...
  if (
    !(
      "beaconApiUrl" in stakingService.token.network &&
      stakingService.token.network.beaconApiUrl
    )
  ) {
  ...
  if (!stakingService.hasBlockRootForTimestamp) {
    throw new Error(
      "Has block root for timestamp is not set for NST verification",
    );
  }
  if (!stakingService.addBlockRootForTimestamp) {
    throw new Error(
      "Add block root for timestamp is not set for NST verification",
    );
  }
  ...
  // Beacon oracle state
  const [timestampAvailable, setTimestampAvailable] = useState<boolean | null>(
    null,
  );
  ...
  const beaconApiUrl = (
    stakingService.token.network as { beaconApiUrl: string }
  ).beaconApiUrl;
  ...
  const fetchSlotsPerEpoch = useCallback(async () => {
    try {
      ...
      console.warn(
        "Failed to fetch slots per epoch from beacon spec, using default value 32:",
        error,
      );
    }
  }, [beaconApiUrl]);
  ...
  const checkTimestampAvailability = useCallback(
    async (timestamp: string) => {
      if (!stakingService.hasBlockRootForTimestamp) return false;
      setIsCheckingTimestamp(true);
      try {
        const isAvailable =
          await stakingService.hasBlockRootForTimestamp(timestamp);
  ...
  const isValidProofData = (
    ...
    return (
      ...
      Array.isArray(
        (data as Record<string, unknown>).validatorContainerProof,
      ) &&
      ...
      ((data as Record<string, unknown>).stateRootProof as unknown[]).every(
        (item: unknown) =>
          typeof item === "string" && item.toString().startsWith("0x"),
      ) &&
      ((data as Record<string, unknown>).validatorContainer as unknown[]).every(
        (item: unknown) =>
          typeof item === "string" && item.toString().startsWith("0x"),
      ) &&
      (
        (data as Record<string, unknown>).validatorContainerProof as unknown[]
      ).every(
        (item: unknown) =>
          typeof item === "string" && item.toString().startsWith("0x"),
      )
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(
            `Validator not found; check the validator public key`,
          );
        }
      ...
      if (validatorData?.validator?.activation_epoch) {
        const activationEpoch = parseInt(
        ...
        try {
          ...
          if (isValidProofData(jsonData)) {
            ...
            const timestampChanged =
              proofData?.timestamp !== jsonData.timestamp;
            if (timestampChanged) {
            ...
            setError(
              "Invalid proof file format. Expected fields: stateRoot, stateRootProof, validatorContainer, validatorContainerProof, slot, validatorIndex, timestamp",
            );
            setProofData(null);
            setTimestampAvailable(null);
            setIsNewTimestamp(false);
  ...
  const handleAddTimestamp = async () => {
    ...
    try {
      ...
      const result = await stakingService.addBlockRootForTimestamp(
        proofData.timestamp,
        {
          onPhaseChange: (phase) => {
            switch (phase) {
  ...
  const handleVerify = async () => {
    ...
    if (timestampAvailable === null) {
      ...
      const isAvailable = await stakingService.hasBlockRootForTimestamp(
        proofData.timestamp,
      );
      if (!isAvailable) {
        // Add timestamp first
    ...
    if (
      validatorIndex !== null &&
      parseInt(proofData.validatorIndex) !== validatorIndex
    ) {
      setError(
        `Proof validator index (${proofData.validatorIndex}) does not match current validator index (${validatorIndex})`,
      );
      return;
    }
    ...
    try {
      ...
      const parsedProof: NSTVerifyParams["proof"] = {
        ...
        stateRootProof: Array.isArray(proofData.stateRootProof)
          ? (proofData.stateRootProof as `0x${string}`[])
          : [],
        validatorContainerRootProof: Array.isArray(
          proofData.validatorContainerProof,
        )
          ? (proofData.validatorContainerProof as `0x${string}`[])
          : [],
      };

      const verifyParams: NSTVerifyParams = {
        validatorContainer: Array.isArray(proofData.validatorContainer)
          ? (proofData.validatorContainer as `0x${string}`[])
          : [],
        proof: parsedProof,
      };
            ...
            <div className="text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {(parseInt(validatorStatus.data.balance) / 1e9).toFixed(4)}{" "}
                  ETH
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {(
                    parseInt(validatorStatus.data.validator.effective_balance) /
                    1e9
                  ).toFixed(4)}{" "}
                  ETH
                </span>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {validatorStatus.data.validator.activation_epoch}
                </span>
              </div>
              {activationSlot !== null && (
                <div className="flex justify-between">
          ...
          <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
            ...
            <div className="text-xs text-green-300 space-y-1">
              ...
              <div>
                Timestamp:{" "}
                {new Date(
                  parseInt(proofData.timestamp) * 1000,
                ).toLocaleString()}
              </div>
              <div>State Root: {proofData.stateRoot.slice(0, 10)}...</div>
              <div>
                Proof Elements:{" "}
                {proofData.stateRootProof.length +
                  proofData.validatorContainerProof.length}
              </div>
            </div>
          </div>
        )}
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-blue-400 text-sm">
                    {isNewTimestamp
                      ? "Checking new timestamp availability..."
                      : "Checking timestamp availability..."}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-green-400 text-sm">
                    Timestamp available in beacon oracle
                  </span>
                </div>
              </div>
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-yellow-400 text-sm">
                    Timestamp not available in beacon oracle - must be added
                    first
                  </span>
                </div>
              </div>
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
        ...
        <ul className="text-blue-300 text-sm space-y-1">
          ...
          <li>
            • Run the binary at{" "}
            <a
              href="https://github.com/imua-xyz/proofs-generation"
              target="_blank"
              rel="noopener noreferrer"
            >
              https://github.com/imua-xyz/proofs-generation
            </a>
          </li>
          <li>
            • Call the server with the validator index and slot{" "}
            {activationSlot !== null && validatorIndex !== null ? (
              <code>
                {"{"}slot: {activationSlot}, validator_index: {validatorIndex}
                {"}"}
              </code>
            ) : (
              ""
