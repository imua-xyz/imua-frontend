...
async function queryLayerZeroStatus(sourceTxHash: string): Promise<{ confirmed: boolean; error?: string; destinationTxHash?: string }> {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS.LAYERZERO; attempt++) {
    try {
      ...
      const response = await fetch(`${LAYERZERO_CONFIG.API_ENDPOINT}${LAYERZERO_CONFIG.PATHS.MESSAGE_STATUS(sourceTxHash)}`);
      
      if (!response.ok) {
        // If it's a 404 or similar, the message might not be indexed yet
        if (response.status === 404) {
          console.log(`LayerZero message not indexed yet, attempt ${attempt}/${MAX_ATTEMPTS.LAYERZERO}`);
          // Wait before next attempt (except on last attempt)
          if (attempt < MAX_ATTEMPTS.LAYERZERO) {
            await new Promise(resolve => setTimeout(resolve, POLLING_INTERVALS.LAYERZERO));
          }
          continue;
        }
        return { confirmed: false, error: `API request failed: ${response.status}` };
      }

      const data = await response.json() as MessageResponse;
      
      if (!data.data || data.data.length === 0) {
        console.log(`No LayerZero message data found, attempt ${attempt}/${MAX_ATTEMPTS.LAYERZERO}`);
        // Wait before next attempt (except on last attempt)
        if (attempt < MAX_ATTEMPTS.LAYERZERO) {
          await new Promise(resolve => setTimeout(resolve, POLLING_INTERVALS.LAYERZERO));
        }
      ...
      if (message.source?.status !== "SUCCEEDED") {
        console.log(`Source transaction not confirmed yet: ${message.source?.status}, attempt ${attempt}/${MAX_ATTEMPTS.LAYERZERO}`);
        // Wait before next attempt (except on last attempt)
        if (attempt < MAX_ATTEMPTS.LAYERZERO) {
          await new Promise(resolve => setTimeout(resolve, POLLING_INTERVALS.LAYERZERO));
        }
      ...
      console.log(`Destination transaction not confirmed yet: ${message.destination?.status}, attempt ${attempt}/${MAX_ATTEMPTS.LAYERZERO}`);
    ...
    if (options?.verifyCompletion) {
      ...
      if (options.getStateSnapshot) {
        try {
          ...
          console.warn("Failed to get state snapshot after verification:", error);
        }
      }

      const isValid = await options.verifyCompletion(snapshotBefore, snapshotAfter);
      if (!isValid) {
...
export async function handleEVMTxWithStatus({
  ...
  try {
    ...
    if (approvingTx) {
      ...
      try {
        ...
        if (approvalReceipt.status !== "success") {
          return { hash: "", success: false, error: "Approval transaction failed" };
        }
      } catch (error) {
    ...
    if (mode === "local") {
      ...
      return await handleCompletion(hash, {
        verifyCompletion,
        getStateSnapshot,
        onPhaseChange,
      }, snapshotBefore);
      ...
      return await handleCrossChainOperation(hash, mode, {
        verifyCompletion,
        getStateSnapshot,
        onPhaseChange,
      }, snapshotBefore);
...
async function handleCrossChainOperation(
  ...
  try {
    ...
    if (!relayResult.confirmed) {
      const errorMessage = relayResult.error || "Cross-chain relay failed";
      return { hash, success: false, error: relayResult.error || "Relay failed" };
    }

    if (mode === "duplex") {
      // Duplex mode: wait for response, pass the destination tx hash
      return await handleDuplexResponse(hash, options, snapshotBefore, relayResult.destinationTxHash);
    } else {
...
async function handleXrplSimplexOperation(
  txHash: string,
  utxoGateway: any,
  options?: Pick<BaseTxOptions, "onPhaseChange" | "verifyCompletion" | "getStateSnapshot">
): Promise<{ hash: string; success: boolean; error?: string }> {
  try {
    ...
    if (!utxoGateway) {
      return { hash: txHash, success: false, error: "UTXOGateway contract not available" };
    }
    ...
    while (!isProcessed && Date.now() - startTime < TIMEOUTS.LAYERZERO_QUERY) {
      try {
        ...
        const processed = await utxoGateway.read.isStakeMsgProcessed([2, bytes32Hash]);
        
        if (processed) {
          isProcessed = true;
          console.log(`XRPL stake message processed after ${Date.now() - startTime}ms`);
          break;
        }
    ...
    return { hash: txHash, success: false, error: "XRPL simplex operation failed" };
  }
}
...
async function handleDuplexResponse(
  ...
  try {
    ...
    if (!destinationTxHash) {
      const errorMessage = "No destination transaction hash found for response verification";
      return { hash, success: false, error: "No destination transaction hash provided for response checking" };
    }
    ...
    if (!responseStatus.confirmed) {
      const errorMessage = responseStatus.error || "Cross-chain response not confirmed";
      return { hash, success: false, error: responseStatus.error || "Response not confirmed" };
    }

    // Response is confirmed, proceed to completion
    console.log(`Duplex response confirmed: ${responseStatus.destinationTxHash}`);
...
export async function handleXrplTxWithStatus({
  ...
  try {
    ...
    if (mode === "local") {
      ...
      await new Promise(resolve => setTimeout(resolve, POLLING_INTERVALS.XRPL));
    }
