...
export function useEVMNSTStaking(token: EVMNSTToken): StakingService {
  ...
  const stakerBalance = useMemo<StakerBalance>(() => {
    ...
    return {
      ...
      withdrawable: s?.withdrawable || BigInt(0), // TODO: we should get withdrawable amount from the capsule contract instead of staker balance
      delegated: s?.delegated || BigInt(0),
      pendingUndelegated: s?.pendingUndelegated || BigInt(0),
      totalDeposited: s?.totalDeposited || BigInt(0),
    };
  }, [stakerBalanceFromHook.data, token, userAddress]);
  ...
  }, [balance.data, token, userAddress]);
  ...
  const handleNSTVerifyAndDeposit = useCallback(
    async (
      ...
      // We get staker balance from indexer, which indexes EVM tx with delay, so we cannot verify completion immediately after tx
      return handleEVMTxWithStatus({
        spawnTx: spawnTx,
  ...
  const handleWithdrawPrincipal = useCallback(
    async (
      ...
      // Simulate the transaction first
      try {
        await simulateTransaction(writeableContract, "withdrawPrincipal", [
          token.address,
          amount,
          recipient,
        ]);
  ...
  const handleDeposit = useCallback(async () => {
    throw new Error(
      "Use nstStake and nstVerifyAndDeposit instead of deposit for NST",
    );
  }, []);

  const handleDepositAndDelegate = useCallback(async () => {
    throw new Error(
      "Use nstStake and nstVerifyAndDeposit instead of depositAndDelegate for NST",
    );
  }, []);

  // Check if capsule exists (read-only)
  const checkCapsuleExists = useCallback(async (): Promise<string | null> => {
    if (!readonlyContract) return null;

    try {
      const existingCapsule = await readonlyContract.read.ownerToCapsule([
        userAddress,
      ]);
      if (
        existingCapsule &&
        existingCapsule !== "0x0000000000000000000000000000000000000000"
      ) {
  ...
  const handleCreateCapsule = useCallback(async (): Promise<{
    address: string;
    txHash: string;
    success: boolean;
    error?: string;
    ...
    const isWhitelisted = await readonlyContract.read.isWhitelistedToken([
      VIRTUAL_NST_ADDRESS,
    ]);
    if (!isWhitelisted) {
      return {
        ...
        success: false,
        error:
          "Native restaking is not enabled - VIRTUAL_NST_ADDRESS is not whitelisted",
      };
    }
    ...
    try {
      // Simulate the transaction first
      try {
        await simulateTransaction(writeableContract, "createImuaCapsule", []);
      } catch (simError) {
        return {
          address: "",
          txHash: "",
      ...
      const result = await handleEVMTxWithStatus({
        spawnTx: () =>
          writeableContract.write.createImuaCapsule([], {
            account: userAddress as `0x${string}`,
      ...
      const capsuleAddress = await readonlyContract.read.ownerToCapsule([
        userAddress,
      ]);
      ...
      const errorMessage =
        directError instanceof Error
          ...
          : String(directError);
      if (
        errorMessage.includes("CapsuleAlreadyCreated") ||
        errorMessage.includes("capsule already exists")
      ) {
        const existingCapsule = await readonlyContract.read.ownerToCapsule([
          userAddress,
        ]);
      ...
      if (
        ...
        return {
          ...
          success: false,
          error:
            "Transaction failed due to RPC error. This might be due to insufficient gas or network issues. Please try again or increase gas limit.",
        };
      }
      return {
        address: "",
  ...
  const handleIsPectraMode = useCallback(
    async (capsuleAddress: string): Promise<boolean> => {
      if (!readonlyContract || !publicClient)
  ...
  const handleAddBlockRootForTimestamp = useCallback(
    async (
      ...
      const beaconOracleAddress =
        await writeableContract.read.BEACON_ORACLE_ADDRESS([]);
      ...
      // Simulate the transaction first
      try {
        await simulateTransaction(beaconOracleContract, "addTimestamp", [
          BigInt(timestamp),
        ]);
      } catch (error) {
        return {
      ...
      const result = await handleEVMTxWithStatus({
        spawnTx: () =>
          beaconOracleContract.write.addTimestamp([BigInt(timestamp)], {
            account: userAddress as `0x${string}`,
          }),
        mode: "local",
        publicClient: publicClient,
  ...
  const handleHasBlockRootForTimestamp = useCallback(
    async (timestamp: string): Promise<boolean> => {
      ...
      return (
        ((await beaconOracleContract.read.timestampToBlockRoot([
          BigInt(timestamp),
        ])) as `0x${string}`) !==
