...
export function useWhitelistedTokens() {
  ...
  const { data: tokenAddresses, isLoading: addressesLoading } =
    useReadContracts({
      contracts: countData
        ? Array.from({ length: Number(countData) }, (_, i) => ({
            address: contractAddress as `0x${string}`,
            abi: contractAbi,
  ...
  const { data: tokenDetails, isLoading: detailsLoading } = useReadContracts({
    contracts: tokenAddresses
      ? (tokenAddresses as any[]).flatMap((response) => {
    ...
    if (address.toLowerCase() === VIRTUAL_TOKEN.toLowerCase()) {
      ...
      const detailsIndex =
        (tokenAddresses as any[])
          ...
          .filter(
            (res) =>
              (res as any).result.toLowerCase() !== VIRTUAL_TOKEN.toLowerCase(),
          ).length * 3;
