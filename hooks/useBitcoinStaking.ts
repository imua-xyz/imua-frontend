...
export function useBitcoinStaking(): StakingService {
  ...
  const psbtBuilder = useBitcoinPSBTBuilder(
  ...
  const bitcoinBindingQuery = useAddressBinding(
    "BTC",
    bitcoinAddress || "",
    tbtc.network.customChainIdByImua,
  );
    ...
    async (
      ...
      const spawnTx = async () => {
        try {
          ...
          // For P2SH addresses, we need to let the wallet handle the redeem script
          const result = await walletProvider.signPSBT({
            psbt: psbt,
            signInputs: [], // Let the wallet sign all inputs and handle redeem scripts
            broadcast: true,
          });
      ...
      if (success && !boundImuaAddress && effectiveAddress && bitcoinAddress) {
        // Set provisional binding that persists across wallet disconnects
        // This binding is bidirectional: Bitcoin address <-> EVM address
        ...
        console.log(
          `Set provisional binding: BTC ${bitcoinAddress} <-> EVM ${effectiveAddress} (chainId: ${tbtc.network.customChainIdByImua})`,
        );

        // Refetch GraphQL binding to sync with database
        if (bitcoinBindingQuery.refetch) {
          bitcoinBindingQuery.refetch().catch(console.error);
        }
      }
