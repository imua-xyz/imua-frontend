...
export function useNSTOperations() {
  ...
  const handleCreateCapsule = useCallback(
    async (options?: TxHandlerOptions) => {
  ...
  const handleStakeToBeacon = useCallback(
    async (
      pubkey: `0x${string}`,
      signature: `0x${string}`,
      ...
      return handleEVMTxWithStatus(
        contract.write.stake([pubkey, signature, depositDataRoot], {
          value: parseEther("32"),
        }),
        options,
      );
  ...
  const handleVerifyAndDepositNativeStake = useCallback(
    async (
  ...
  const handleProcessBeaconWithdrawal = useCallback(
    async (
      validatorContainer: `0x${string}`[],
      validatorProof: any,
  ...
  const handleWithdrawNonBeaconETH = useCallback(
    async (
      recipient: `0x${string}`,
      amount: bigint,
      options?: TxHandlerOptions,
      ...
      return handleEVMTxWithStatus(
        contract.write.withdrawNonBeaconChainETHFromCapsule([
          recipient,
          amount,
        ]),
        options,
