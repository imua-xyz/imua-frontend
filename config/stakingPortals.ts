...
export const getMetadataByEvmChainID = (evmChainID: number): StakingProviderMetadata | undefined => {
...
export const getMetadataByCustomChainID = (customChainID: number): StakingProviderMetadata | undefined => {
    const portal = Object.values(stakingPortals).find((portal) => {
...
export const getPortalContractByEvmChainID = (evmChainID: number): {
    name: string,
...
export const getAvailableNetworks = (): string[] => {
    return Object.values(stakingPortals).filter((portal) => {
        return portal.portalContracts[0].address !== null
    }).map((portal) => {return portal.name})
}

export const getCustomChainIdByEvmChainID = (evmChainID: number): number | null => {
    const portal = Object.values(stakingPortals).find((portal) => {
        if (portal.evmChainID === evmChainID) {
            return portal
        }
